    ...
    <motion.div
      ...
      <div className="relative h-48 w-full bg-slate-900 flex items-center justify-center overflow-hidden">
        ...
        {item.images && item.images.length > 0 ? (
          ...
          <span className="text-xs font-semibold tracking-wider text-slate-500 uppercase group-hover:text-blue-400 transition-colors cursor-default">
            {item.category_detail?.name || "Ürün Görseli"}
          </span>
        )}

        <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-md text-[11px] font-medium text-slate-300 px-2.5 py-1 rounded-lg border border-slate-700/80 z-10 cursor-default">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-700/50">
          ...
          <button
            ...
            className="btn-slate hover:bg-blue-600 hover:text-white hover:border-blue-500 cursor-pointer active:scale-95 transition-all shadow-sm">
            İncele
          </button>
        </div>
      </div>
