...
import { Link } from "react-router-dom"; // 🎯 EKLENDİ: useNavigate yerine Link kullanıyoruz
...
const ItemCard = ({ item }) => {
  ...
  const handleFavoriteClick = async (e) => {
    e.preventDefault(); // 🎯 YENİ: Kalbe basınca ilana gitmesini engeller
    e.stopPropagation(); // 🎯 YENİ: Link'in tıklanma eventini durdurur

    const token = localStorage.getItem("access_token");
    if (!token) return toast.fire({ icon: "info", title: "Favoriye eklemek için giriş yapmalısınız." });

    try {
      await itemApi.toggleFavorite(item.id, token);
      setIsFavorite(!isFavorite);
      toast.fire({ icon: "success", title: !isFavorite ? "Favorilere eklendi." : "Favorilerden çıkarıldı." });
    } catch (err) {
      toast.fire({ icon: "error", title: "Favori işlemi başarısız oldu." });
    }
  };
  ...
  return (
    <motion.div
      ...
      {/* 🎯 DÜZELTİLDİ: Tüm kartı Link ile sardık. Blok element olması için 'block' class'ı eklendi */}
      <Link
        to={`/listings/${item.id}`}
        className="group relative cyber-card overflow-hidden hover:shadow-blue-500/10 hover:border-blue-500/30 block cursor-pointer">
        {/* Görsel Alanı */}
        <div className="relative h-48 w-full bg-slate-900 flex items-center justify-center overflow-hidden">
          ...
          <button
            ...
            className="absolute top-3 right-3 z-20 p-2 rounded-xl bg-slate-950/80 backdrop-blur-md border border-slate-700/80 text-xs transition-all cursor-pointer active:scale-90 hover:scale-110 hover:border-rose-500/50 group/heart">
            <span
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-700/50">
            ...
            {/* 🎯 DÜZELTİLDİ: Buton artık bir div gibi görünüyor çünkü kapsayıcı zaten bir Link */}
            <div className="btn-slate hover:bg-blue-600 hover:text-white hover:border-blue-500 transition-all shadow-sm">İncele</div>
          </div>
        </div>
      </Link>
    </motion.div>
